...
"""
옵션 A: 지표 중심 구조
상위 헤더: 기본정보 | 순위 | 순위변화 | 가격 | 가격변화 | 할인율 | 할인율변화
하위 헤더: - | 10/20,10/21,10/23 | 10/21,10/23 | ...
...
def generate_metric_centered_format():
    """지표 중심 구조로 Wide Format 생성"""
    ...
    for d in dates:
        multi_columns.append(('순위', d[5:].replace('-', '/')))
    for i in range(1, len(dates)):
        multi_columns.append(('순위변화', dates[i][5:].replace('-', '/')))
    
    for d in dates:
        multi_columns.append(('가격', d[5:].replace('-', '/')))
    for i in range(1, len(dates)):
        multi_columns.append(('가격변화', dates[i][5:].replace('-', '/')))
    
    for d in dates:
        multi_columns.append(('할인율(%)', d[5:].replace('-', '/')))
    for i in range(1, len(dates)):
        multi_columns.append(('할인율변화', dates[i][5:].replace('-', '/')))
...
def main():
    ...
    print("🎯 옵션 A: 지표 중심 구조")
    print("="*80)
    print("상위 헤더: 기본정보 | 순위 | 순위변화 | 가격 | 가격변화 | 할인율 | 할인율변화")
    print("하위 헤더: - | 10/20,10/21,10/23 | 10/21,10/23 | ...")
    print("\n특징:")
    print("  ✓ 지표별 그룹화 → 지표 간 비교 용이")
    print("  ✓ 모든 변화량: 절대치")
    print("  ✓ 변화량 색상: 음수=파랑, 양수=빨강")
    print("  ✓ 미니멀 디자인")
    print("="*80)
    
    dates, wide_df = generate_metric_centered_format()
    
    output_path = os.path.join(OUTPUT_DIR, "option_A_metric_centered.xlsx")
    save_to_excel_minimal(dates, wide_df, output_path, "A")
    
    # CSV
    csv_path = os.path.join(OUTPUT_DIR, "option_A_metric_centered.csv")
